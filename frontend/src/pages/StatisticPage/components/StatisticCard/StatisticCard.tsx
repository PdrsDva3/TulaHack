...
export const StatisticCard: React.FC<StatisticCardProps> = ({ title, series, palette, desc}) => {
	return (
		<Container
			sx={{
		...
		>
			...
			<Box sx={{display:"flex", justifyContent:"center", alignItems:"center"}}>
				...
				<Box sx={{display:"flex", flexDirection:"column", gap:2}}>
					{data.map((item, index) => {
						return(
							<Chip key={index} label={`${item.value} ${desc[index]}`} sx={{backgroundColor: palette[index], color:"common.white"}} />
					)}
					)}
				</Box>
			</Box>
